...
# Устанавливаем системные шрифты и chromium для 100% точной генерации PDF
RUN apk add --no-cache font-dejavu chromium harfbuzz nss freetype ttf-freefont

# Устанавливаем рабочую директорию внутри контейнера
WORKDIR /app

# Копируем скомпилированный исполняемый JAR-файл
COPY server/build/libs/server-1.0.3.jar app.jar
