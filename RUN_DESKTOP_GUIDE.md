...
> [!NOTE]
> Установка Docker является полностью опциональной. Если вы хотите запускать сервер напрямую на компьютере, вам достаточно установить только Java 21 (Шаг 1.1), а Шаг 1.2 (Docker) можно полностью пропустить.
...
* **Docker** — если вы хотите запускать сервер в изолированном контейнере (опционально).
...
### 1.2. Установка Docker (Опционально — только если вы хотите использовать Docker)
* **Для Windows и macOS**:
...
## ШАГ 5. Настройка каналы доставки чеков (Опционально)

В режиме DESKTOP все настройки каналов доставки (Email, Telegram, WhatsApp, SMS, Сетевой принтер) хранятся в простом текстовом файле **`config/core-settings.json`** относительно рабочей директории запуска.

Вы можете настроить каналы доставки двумя способами:

### Способ А. Редактирование файла `config/core-settings.json` вручную
Вы можете открыть файл `config/core-settings.json` в любом текстовом редакторе (Блокнот, VS Code) и прописать нужные реквизиты.

Пример заполненного блока `delivery` в файле настроек:
```json
{
  "deliveryChannels": ["PRINT", "EMAIL", "TELEGRAM"],
  "delivery": {
    "channels": [
      { "channel": "PRINT", "enabled": true },
    ...
    "email": {
      "host": "smtp.mail.ru",
      "port": 465,
      "user": "my_kassa@mail.ru",
      "password": "my_smtp_password",
      "from": "my_kassa@mail.ru"
    },
    "telegram": {
...
*После сохранения файла сервер автоматически перечитает новые настройки на лету.*

---

### Способ Б. Изменение настроек через Swagger UI (REST API)
Вместо ручного редактирования файла на диске вы можете использовать графический интерфейс Swagger:
1. Вызовите метод **`GET /settings`** (он прочитает текущее содержимое файла `config/core-settings.json`).
2. Скопируйте полученный JSON-ответ в буфер обмена.
3. Откройте метод **`PUT /settings`**, вставьте JSON в поле запроса, измените параметры (например, введите хост принтера или токен бота) и нажмите **Execute**.
4. Сервер автоматически обновит настройки в памяти и **самостоятельно перезапишет файл `config/core-settings.json`** на диске.
