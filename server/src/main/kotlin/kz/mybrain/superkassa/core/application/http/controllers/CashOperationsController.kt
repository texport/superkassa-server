...
class CashOperationsController(private val kkmService: SuperkassaApi) {
     ...
     * Сумма автоматически преобразуется из тенге в формат ОФД (bills/coins).
     */
    ...
    @Operation(
        ...
        description = """
            ...
            - **amount** (обязательно): Сумма внесения в тенге, точным десятичным числом.
    ...
    @Operation(
        ...
        description = """
            ...
            - **pin** (обязательно): ПИН-код пользователя для авторизации операции.
              Должен соответствовать пользователю с правами CASHIER или ADMIN.
            - **amount** (обязательно): Сумма изъятия в тенге, точным десятичным числом.
              Например: 1234.56 означает 1234 тенге 56 тиынов.
              Система автоматически преобразует сумму в формат ОФД (bills/coins):
              * bills = целая часть (тенге)
              * coins = дробная часть * 100 (тиыны)
