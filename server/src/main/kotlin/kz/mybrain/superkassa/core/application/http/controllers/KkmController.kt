...
class KkmController(
    ...
    @Operation(
        ...
        description = """
            Отправляет чек покупателю сразу, не дожидаясь фоновой досылки узла.

            **Особенности отправки:**
            - Идёт по тем же задачам доставки, что и фон: каналы и получатель — из контакта покупателя в чеке.
            - Доставленное второй раз не уходит; задачу, которую сейчас отправляет фон, повтор не трогает.
            - Без контакта покупателя в чеке цифровые каналы его не получают; печать на сетевой принтер — по настройкам.
