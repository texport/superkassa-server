...
class SuperkassaSettingsController(
    ...
    @Operation(
        ...
        description = """
            ...
            **ofdTimeoutSeconds** - Сколько касса ждёт ответа БФД:
            - Не менее 5 сек. По умолчанию: 7 сек.
            - Соединение, отправка и чтение укладываются в этот же срок.
            - По истечении чек оформляется автономно и встаёт в очередь отправки.
    ...
    @Operation(
        ...
        description = """
            ...
            **Важные замечания:**
            ...
            - ofdProtocolVersion задаётся при запуске узла и через этот метод не меняется:
              попытка прислать другое значение отвергается кодом SETTING_IS_DEPLOYMENT_OWNED.
