...
class VatRatesController(private val kkmService: SuperkassaApi) {
    ...
    @Operation(
        ...
        description = """
            ...
            - `code`: уникальный код группы НДС (например, `VAT_12`)
            - `percent`: процентная ставка (в целых процентах, например, 12)
            - `description`: понятное текстовое описание ставки (например, `НДС 12%`)
            - `name`: трехязычное название с локализацией на русский, казахский и английский языки
