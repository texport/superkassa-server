 ...
 * Общая сумма чека (total) не передаётся — вычисляется на сервере по позициям и скидке/наценке на чек.
 * Порядок полей: idempotencyKey, items, markupPercent, markupSum, discountPercent, discountSum, change, defaultVatGroup, payments, taken.
 */
@Serializable
