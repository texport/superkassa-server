...
class SystemControllersTest {
    ...
    fun `superkassa info controller returns kkm page and system info`() {
        ...
        val list = controller.listKkms(limit = 50, offset = 0, state = null, search = null, sortBy = "createdAt", order = "DESC")
    ...
    fun `superkassa settings controller gets and updates settings when allowed`() {
        ...
        val repo = mockk<FileCoreSettingsRepository>()
    ...
    fun `diagnostics health degrades when ofd check fails`() {
        ...
        every { kkmService.checkOfdConnection("kkm-2") } returns OfdCommandResult(status = OfdCommandStatus.FAILED, errorMessage = "down")
