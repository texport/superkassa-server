...
/**
 * Адаптер StoragePort на базе библиотеки superkassa-storage.
    ...
    private fun <T> withSession(block: (StorageSession) -> T): T {
        try {
            ...
            throw StorageException(
                TrilingualMessage(
                    ru = "Ошибка базы данных: ${e.message}",
                    kk = "Деректер қорының қатесі: ${e.message}",
                    en = "Database error: ${e.message}"
                ),
                cause = e
            )
        }
